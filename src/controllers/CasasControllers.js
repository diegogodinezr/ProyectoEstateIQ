...
class CasasController{
    ...
    static async indexPost(req, res) {
        try {
            ...
            const insertedId = await CasasModel.insertar(newData);

            res.status(201)
                .header('Location', `/casas/${insertedId}`)
    ...
    static async itemPut(req, res) {
        try {
            ...
            const result = await CasasModel.reemplazar(id, updatedData);
    ...
    static async itemPatch(req, res) {
        try {
            ...
            const result = await CasasModel.actualizar(id, updatedFields);
