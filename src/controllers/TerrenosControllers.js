...
class TerrenosController{
    ...
    static async indexPost(req, res) {
        try {
            ...
            const insertedId = await TerrenosModel.insertar(newData);

            res.status(201)
                .header('Location', `/terrenos/${insertedId}`)
    ...
    static async itemPut(req, res) {
        try {
            ...
            const result = await TerrenosModel.reemplazar(id, updatedData);
    ...
    static async itemPatch(req, res) {
        try {
            ...
            const result = await TerrenosModel.actualizar(id, updatedFields);
